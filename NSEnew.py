...
def _power_scan_volatile(threshold, period, top_n, use_smp=False):
    ...
    _picked = volatile[:top_n]
    try:
        _tracker_ingest(_picked)
    except Exception:
        pass
    return _picked, bt_out, nifty50_state(), ""
...
def _tracker_load() -> list:
    ...
    Merges Gist cloud storage + local JSON file (union by symbol+scan_date)
    so an empty/stale Gist never hides locally imported data.
    """
    ...
    try:
        ...
        if _TRACKER_FILE.exists():
            with open(_TRACKER_FILE, "r") as f:
                d = json.load(f)
            local_data = d if isinstance(d, list) else []
    except Exception:
        local_data = []

    merged: dict = {}
    for rec in (local_data or []):
        k = (rec.get("symbol",""), rec.get("scan_date",""))
        merged[k] = rec
    for rec in (gist_data or []):
        k = (rec.get("symbol",""), rec.get("scan_date",""))
        merged[k] = rec   # gist wins on conflict (assumed freshest)

    result = list(merged.values())

    # Keep local file in sync with the merged view
    try:
        _TRACKER_FILE.parent.mkdir(parents=True, exist_ok=True)
...
def _sa_prices_batch(sym_tuple):
    """
    Fetch latest close for a tuple of NSE symbols using batch yf.download.
    Decorated with @st.cache_data(ttl=300) — refreshes every 5 min.
    Batch download is 10-20x faster than one-by-one Ticker calls.
    """
    import io as _io_sa, contextlib as _cl_sa
    prices = {}
    if not sym_tuple:
        return prices
    # Build NSE ticker strings
    ns_tickers = [yf_ticker(s) for s in sym_tuple]
    try:
        buf = _io_sa.StringIO()
        with _cl_sa.redirect_stdout(buf), _cl_sa.redirect_stderr(buf):
            df = yf.download(
                " ".join(ns_tickers),
                period="3d",
                interval="1d",
        ...
        if isinstance(df.columns, pd.MultiIndex):
            for ns_t, sym in zip(ns_tickers, sym_tuple):
                try:
                    col = (df[ns_t]["Close"] if ns_t in df.columns.get_level_values(0)
                           else df["Close"][ns_t] if ("Close", ns_t) in df.columns else None)
                    if col is not None:
                        last = col.dropna()
                        if len(last) > 0:
                            px = float(last.iloc[-1])
                            if px == px:
            ...
            if "Close" in df.columns and len(sym_tuple) == 1:
                last = df["Close"].dropna()
                if len(last) > 0:
                    px = float(last.iloc[-1])
                    if px == px:
                        prices[sym_tuple[0]] = round(px, 2)
    except Exception:
        pass

    # Fallback: fetch any remaining symbols individually
    missing = [s for s in sym_tuple if s not in prices]
    for sym in missing[:30]:   # cap at 30 to avoid timeout
        try:
            buf = _io_sa.StringIO()
            with _cl_sa.redirect_stdout(buf), _cl_sa.redirect_stderr(buf):
                h = yf.Ticker(yf_ticker(sym)).history(period="2d", auto_adjust=True)
            if h is not None and len(h) > 0:
                close_col = h.get("Close", h.get("close"))
...
# ══════════════════════════════════════════════════════════════════
    ...
    with tabs[8]:
        ...
        # ── Load tracked signals ──────────────────────────────────────────
        if "tracker_data_cache" not in st.session_state:
            st.session_state["tracker_data_cache"] = _tracker_load()
        _tracked = st.session_state["tracker_data_cache"]
                    ...
                    try:
                        ...
                        if not isinstance(_import_data, list):
                            ...
                            if _added > 0:
                                ...
                                st.session_state["tracker_data_cache"] = _tracked
...
""")
        ...
        if not _tracked:
            ...
            st.session_state["tracker_data_cache"] = _tracked  # refresh analytics cache
    ...
    # ══════════════════════════════════════════════════════════════════════
    # TAB 9 — STRATEGY ANALYTICS
    # ══════════════════════════════════════════════════════════════════════
    with tabs[9]:
        # Always reload — never depend on session cache to avoid blank tab
        if st.button("🔁 Force Reload Tracker Data", key="sa_force_reload"):
            st.cache_data.clear()
        _sa_tracked = _tracker_load()
        ...
        <div style='padding:12px 18px;background:linear-gradient(90deg,#38bdf820,#26a69a10,#0b0e11);
          ...
          <div style='font-family:Syne,sans-serif;font-weight:800;font-size:1rem;color:#38bdf8'>
            📉 Strategy Analytics — What Is Working &amp; What Is Not
          </div>
          <div style='font-size:.74rem;color:#787b86;margin-top:3px'>
            Every tracked signal analysed by: Pattern · Sector · AI Band · RSI Zone · ADX · Hold Duration · Source
          </div>
        </div>""", unsafe_allow_html=True)

        if not _sa_tracked:
            st.info(
                f"📭 No tracker data found (checked Gist + local file at `{_TRACKER_FILE}`).",
                icon="📡"
            )
            _sa_upload = st.file_uploader(
                "Upload tracker.json directly here to populate analytics",
                type=["json"], key="sa_direct_upload"
            )
            if _sa_upload:
                try:
                    _sa_imported = json.load(_sa_upload)
                    if isinstance(_sa_imported, list) and _sa_imported:
                        _tracker_save(_sa_imported)
                        st.success(f"✅ Loaded {len(_sa_imported)} records. Click below to refresh.")
                        if st.button("🔄 Refresh Analytics Now", key="sa_refresh_after_upload"):
                            st.rerun()
                    else:
                        st.error("File must be a non-empty JSON array of signal records.")
                except Exception as _sa_err:
                    st.error(f"Could not read file: {_sa_err}")
        else:
            # ── Step 1: fetch live prices ─────────────────────────────
            _sa_ref_btn = st.button("🔄 Refresh Live Prices", key="sa_ref")
            if _sa_ref_btn:
                st.cache_data.clear()

            _sa_syms = tuple(sorted({r["symbol"] for r in _sa_tracked}))
            _sa_live = {}
            if _sa_syms:
                try:
                    with st.spinner(f"Loading live prices for {len(_sa_syms)} symbols…"):
                        _sa_live = _sa_prices_batch(_sa_syms)
                except Exception:
                    _sa_live = {}
            # Always proceed — show closed-signal analytics even without live prices

            # ── Step 2: build enriched rows with P&L for every record ─
            def _resolve_pnl(rec, live_px):
                ep   = _safe_float(rec.get("entry_price"), 0)
                stp  = _safe_float(rec.get("st_target"),   0)
                sl   = _safe_float(rec.get("st_sl"),        0)
                cp   = _safe_float(rec.get("close_price"),  None)
                stat = rec.get("status", "OPEN")
                live = live_px.get(rec["symbol"])

                # Priority: live > close_price > status estimate
                if live and ep > 0:
                    ref = live
                elif cp and ep > 0:
                    ref = cp
                elif stat == "TARGET_HIT" and stp > 0 and ep > 0:
                    ref = stp
                elif stat == "SL_HIT" and sl > 0 and ep > 0:
                    ref = sl
                else:
                    ref = ep   # no data → 0% change

                return round((ref / ep - 1) * 100, 2) if ep > 0 else 0.0, ref

            _sa_rows = []
            for rec in _sa_tracked:
                pnl, ref_price = _resolve_pnl(rec, _sa_live)
                stat = rec.get("status", "OPEN")
                ep   = _safe_float(rec.get("entry_price"), 0)
                stp  = _safe_float(rec.get("st_target"),   0)
                sl   = _safe_float(rec.get("st_sl"),        0)
                live = _sa_live.get(rec["symbol"])
                # auto-update status
                if stat == "OPEN" and live and ep > 0:
                    if stp > 0 and live >= stp: stat = "TARGET_HIT"
                    elif sl  > 0 and live <= sl: stat = "SL_HIT"
                try:
                    import datetime as _dt3
                    hd = (datetime.now() - _dt3.datetime.strptime(rec["scan_date"],"%Y-%m-%d")).days
                except Exception:
                    hd = 0
                _sa_rows.append(dict(
                    ...
                    _status    = stat,
                    _hold_days = hd,
                    _scan_date = rec.get("scan_date",""),
                    _ai        = _safe_float(rec.get("ai_score"), 0),
                    _rsi       = _safe_float(rec.get("rsi"), None),
                    _adx       = _safe_float(rec.get("adx"), None),
                    _vol       = _safe_float(rec.get("vol_ratio"), None),
                    _top_signal= (rec.get("top_signal","") or "").strip(),
                    _sector    = (rec.get("sector","N/A") or "N/A").strip(),
                    _is_fo     = rec.get("is_fo", False),
                    _method    = rec.get("added_method","auto"),
                ))

            n_total = len(_sa_rows)

            # ── Step 3: stats helper ──────────────────────────────────
            def _stats(rows):
                if not rows: return None
                pnls  = [_safe_float(r["_pnl"], 0) for r in rows]
                n     = len(pnls)
                ...
                pf    = round(abs(aw*len(wins)/(al*len(loss))), 2) if loss and al != 0 else None
                return dict(n=n, avg=avg, wr=wr, best=best, worst=worst, aw=aw, al=al, pf=pf)

            # ── Step 4: render helper ─────────────────────────────────
            def _render_table(title, icon, groups, min_n=1):
                rows = []
                ...
                if not rows:
                    st.caption(f"Not enough data for {title} (need ≥{min_n} signals per group).")
                    return
                rows.sort(key=lambda x: -x[1]["avg"])

                # Header
                st.markdown(
                    f"<div style='display:grid;grid-template-columns:190px 90px 130px 55px 65px 70px 70px 80px 95px;"
                    f"gap:0;padding:4px 10px;margin:14px 0 2px;background:#1c2030;border-radius:3px'>"
                    + "".join(f"<span style='font-size:.6rem;color:#434651;text-transform:uppercase;letter-spacing:.05em'>{h}</span>"
                ...
                for lbl, s in rows:
                    c   = "#26a69a" if s["avg"] >= 0 else "#ef5350"
                    wc  = "#26a69a" if s["wr"] >= 55 else "#f59e0b" if s["wr"] >= 40 else "#ef5350"
                    pfc = "#26a69a" if (s["pf"] or 0) >= 1.5 else "#f59e0b" if (s["pf"] or 0) >= 1 else "#ef5350"
                    vrd = ("✅ WORKING" if s["avg"]>=3 and s["wr"]>=55 else
                           "🟢 Positive" if s["avg"]>=0 and s["wr"]>=45 else
                           "🟡 Mixed"   if s["avg"]>=-2 else "🔴 Avoid")
                    vc  = "#26a69a" if "✅" in vrd else "#4db6ac" if "🟢" in vrd else "#f59e0b" if "🟡" in vrd else "#ef5350"
                    bw  = _safe_int(min(abs(s["avg"])/10*110, 110), 0)
                    pf_str = f"PF {s['pf']:.2f}" if s["pf"] else "PF —"
                    st.markdown(
                        f"<div style='display:grid;grid-template-columns:190px 90px 130px 55px 65px 70px 70px 80px 95px;"
                        f"gap:0;align-items:center;padding:6px 10px;margin-bottom:2px;"
                        f"background:#131722;border-left:3px solid {c};border-radius:3px'>"
                        f"<span style='font-size:.79rem;color:#d1d4dc;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-right:6px'>{str(lbl)[:28]}</span>"
                        f"<span style='font-size:.72rem;font-weight:700;color:{vc}'>{vrd}</span>"
                        f"<div style='display:flex;align-items:center;gap:6px'>"
                        f"<div style='height:6px;width:{bw}px;background:{c};border-radius:3px;flex-shrink:0'></div>"
                        f"<span style='font-size:.82rem;font-weight:700;color:{c}'>{s['avg']:+.1f}%</span>"
                        f"</div>"
                        f"<span style='font-size:.72rem;color:#434651;text-align:center'>{s['n']}</span>"
                        f"<span style='font-size:.78rem;font-weight:600;color:{wc};text-align:center'>{s['wr']:.0f}%</span>"
                        ...
                        f"<span style='font-size:.72rem;color:{pfc};text-align:center'>{pf_str}</span>"
                        f"<span style='font-size:.68rem;color:#434651'>W:{s['aw']:+.1f}% L:{s['al']:+.1f}%</span>"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
            ...
            _all_s = _stats(_sa_rows)
            if _all_s:
                _oc = "#26a69a" if _all_s["avg"] >= 0 else "#ef5350"
                _ov = ("✅ Strategy PROFITABLE — keep running scans"
                       if _all_s["avg"]>=3 and _all_s["wr"]>=55 else
                       "🟢 Positive edge — refine entry criteria"
                       if _all_s["avg"]>=0 and _all_s["wr"]>=45 else
                       "🟡 Mixed results — tighten AI/RSI filters"
                       if _all_s["avg"]>=-2 else
                       "🔴 Negative overall — review signal criteria")
                st.markdown(
                    f"<div style='background:#131722;border:1px solid {_oc}55;"
                    f"border-left:4px solid {_oc};border-radius:5px;padding:14px 18px;margin-bottom:16px'>"
                    f"<div style='font-family:Syne,sans-serif;font-weight:800;font-size:1rem;"
                    f"color:{_oc};margin-bottom:12px'>{_ov}</div>"
                    f"<div style='display:grid;grid-template-columns:repeat(7,1fr);gap:10px'>"
                    + "".join(
                        f"<div style='text-align:center'>"
                        f"<div style='font-size:.6rem;color:#434651;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px'>{_l}</div>"
                        f"<div style='font-family:Syne,sans-serif;font-weight:700;color:{_c};font-size:.95rem'>{_v}</div>"
                        f"</div>"
                        for _l,_v,_c in [
                            ("Signals",    str(_all_s["n"]),                             "#a855f7"),
                            ("Win Rate",   f"{_all_s['wr']:.1f}%",                      "#26a69a" if _all_s["wr"]>=55 else "#f59e0b"),
                            ("Avg P&L",    f"{_all_s['avg']:+.2f}%",                    _oc),
                            ("Best",       f"+{_all_s['best']:.1f}%",                   "#26a69a"),
                            ("Worst",      f"{_all_s['worst']:.1f}%",                   "#ef5350"),
                            ("Profit Factor", f"{_all_s['pf']:.2f}" if _all_s["pf"] else "—",
                             "#26a69a" if (_all_s["pf"] or 0)>=1.5 else "#f59e0b"),
                            ("Avg W/L",    f"{_all_s['aw']:+.1f}% / {_all_s['al']:+.1f}%", "#787b86"),
                        ]
                    )
                    + "</div></div>",
                    unsafe_allow_html=True,
            ...
            # ── Legend row ────────────────────────────────────────────
            st.markdown(
                "<div style='display:flex;gap:16px;flex-wrap:wrap;padding:6px 10px;"
                "background:#1c2030;border-radius:4px;margin-bottom:10px;font-size:.68rem;color:#787b86'>"
                "<span><b style='color:#26a69a'>✅ WORKING</b> avg ≥ +3% &amp; WR ≥ 55%</span>"
                "<span><b style='color:#4db6ac'>🟢 Positive</b> avg ≥ 0% &amp; WR ≥ 45%</span>"
                "<span><b style='color:#f59e0b'>🟡 Mixed</b> avg ≥ −2%</span>"
                "<span><b style='color:#ef5350'>🔴 Avoid</b> avg &lt; −2%</span>"
                "<span style='margin-left:auto'>PF = Profit Factor · WR = Win Rate · W/L = Avg Win / Avg Loss</span>"
                "</div>",
                unsafe_allow_html=True,
            )

            # Note if live prices unavailable
            _n_with_live = sum(1 for r in _sa_rows if _sa_live.get(r["_sym"]))
            _n_closed    = sum(1 for r in _sa_rows if r["_status"] in ("TARGET_HIT","SL_HIT","CLOSED"))
            if _n_with_live == 0 and _n_closed == 0:
                st.warning(
                    "⚠️ Live prices could not be fetched and no signals are marked as closed yet. "
                    "P&L for OPEN signals shows 0%. "
                    "Click **🔄 Refresh Live Prices** to load current prices, or "
                    "mark closed signals in the **Signal Tracker** tab.",
                    icon="⚠️"
                )
            elif _n_with_live == 0:
                st.info(
                    f"ℹ️ Live prices unavailable — showing P&L for {_n_closed} closed signals only. "
                    "Click 🔄 Refresh Live Prices to load current prices.",
                    icon="ℹ️"
                )
            else:
                st.caption(
                    f"✅ Live prices loaded for {_n_with_live} symbols · "
                    f"{_n_closed} closed signals · "
                    f"Total: {len(_sa_rows)} signals analysed"
                )

            # ── 1. By Signal Pattern ──────────────────────────────────
            _g1 = {}
            for r in _sa_rows:
                _raw = (r["_top_signal"] or "").split("·")[0].strip()
                # Remove leading emoji characters (unicode category So/Sm)
                import unicodedata as _ud
                _clean = ""
                _skip  = True
                for _ch in _raw:
                    if _skip and (_ud.category(_ch) in ("So","Sm","Sk","Mn","Cs") or ord(_ch) > 0x2000):
                        continue
                    _skip = False
                    _clean += _ch
                s = _clean.strip()[:38] if _clean.strip() else _raw[:38]
                if not s: s = "No Signal Data"
                _g1.setdefault(s, []).append(r)

            _sec1, _sec2 = st.columns(2)
            with _sec1:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>🎯 By Signal Pattern</div>", unsafe_allow_html=True)
                _render_table("Signal Pattern", "🎯", _g1, min_n=1)

            # ── 2. By Sector ──────────────────────────────────────────
            _g2 = {}
            for r in _sa_rows:
                _g2.setdefault(r["_sector"] or "N/A", []).append(r)
            with _sec2:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>🏭 By Sector</div>", unsafe_allow_html=True)
                _render_table("Sector", "🏭", _g2, min_n=1)

            # ── 3. By AI Score ────────────────────────────────────────
            _g3 = {}
            for r in _sa_rows:
                ai = _safe_float(r["_ai"], 0)
                bk = ("AI ≥ 80% — Very High" if ai>=80 else
                      "AI 65–79% — High"     if ai>=65 else
                      "AI 50–64% — Medium"   if ai>=50 else
                      "AI < 50% — Low")
                _g3.setdefault(bk, []).append(r)

            # ── 4. By RSI Zone ────────────────────────────────────────
            _g4 = {}
            for r in _sa_rows:
                rv = _safe_float(r["_rsi"], 50) or 50
                bk = ("RSI < 30  Deeply Oversold" if rv<30 else
                      "RSI 30–39 Oversold"         if rv<40 else
                      "RSI 40–49 Recovering"        if rv<50 else
                      "RSI 50–64 Neutral"           if rv<65 else
                      "RSI ≥ 65  Elevated")
                _g4.setdefault(bk, []).append(r)

            _sec3, _sec4 = st.columns(2)
            with _sec3:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>🤖 By AI Score Band</div>", unsafe_allow_html=True)
                _render_table("AI Band", "🤖", _g3, min_n=1)
            with _sec4:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>📉 By RSI Zone at Signal</div>", unsafe_allow_html=True)
                _render_table("RSI Zone", "📉", _g4, min_n=1)

            # ── 5. By ADX ─────────────────────────────────────────────
            _g5 = {}
            for r in _sa_rows:
                av = _safe_float(r["_adx"], 0) or 0
                bk = ("ADX ≥ 40 Very Strong" if av>=40 else
                      "ADX 28–39 Strong"      if av>=28 else
                      "ADX 20–27 Moderate"    if av>=20 else
                      "ADX < 20  Weak/Sideways")
                _g5.setdefault(bk, []).append(r)

            # ── 6. By Hold Duration ───────────────────────────────────
            _g6 = {}
            for r in _sa_rows:
                hd = _safe_float(r["_hold_days"], 0) or 0
                bk = ("0–3 days  Very Short" if hd<=3 else
                      "4–7 days  Short"       if hd<=7 else
                      "8–14 days Medium"       if hd<=14 else
                      "15–30 days Swing"       if hd<=30 else
                      "30+ days  Long Hold")
                _g6.setdefault(bk, []).append(r)

            _sec5, _sec6 = st.columns(2)
            with _sec5:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>💪 By ADX Strength</div>", unsafe_allow_html=True)
                _render_table("ADX", "💪", _g5, min_n=1)
            with _sec6:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>⏱️ By Hold Duration</div>", unsafe_allow_html=True)
                _render_table("Hold", "⏱️", _g6, min_n=1)

            # ── 7. Source + F&O ───────────────────────────────────────
            _g7 = {}
            for r in _sa_rows:
                _g7.setdefault(r["_method"].replace("-"," ").title(), []).append(r)
            _g8 = {}
            for r in _sa_rows:
                _g8.setdefault("F&O Eligible" if r["_is_fo"] else "Non-F&O (Cash)", []).append(r)

            _sec7, _sec8 = st.columns(2)
            with _sec7:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>⚙️ Auto vs Manual Signals</div>", unsafe_allow_html=True)
                _render_table("Source", "⚙️", _g7, min_n=1)
            with _sec8:
                st.markdown("<div style='font-size:.82rem;font-weight:700;color:#38bdf8;padding:4px 0 8px;border-bottom:1px solid #1c2030;margin-bottom:2px'>🔰 F&O vs Non-F&O Stocks</div>", unsafe_allow_html=True)
                _render_table("F&O", "🔰", _g8, min_n=1)

            # ── 8. Top 10 best / worst ────────────────────────────────
            st.markdown(
                "<div style='font-size:.82rem;font-weight:700;color:#d1d4dc;"
                "padding:10px 0 6px;border-bottom:1px solid #1c2030;margin:16px 0 8px'>"
                "🏆 Top 10 Best Signals &nbsp;|&nbsp; 💀 Top 10 Worst Signals</div>",
                unsafe_allow_html=True,
            )
            _sorted_all = sorted(_sa_rows, key=lambda r: -_safe_float(r["_pnl"], -999))
            _top10 = _sorted_all[:10]
            _bot10 = _sorted_all[-10:][::-1]

            _tc, _bc = st.columns(2)
            for _cw, _items in [(_tc, _top10), (_bc, _bot10)]:
                with _cw:
                    for ix, r in enumerate(_items, 1):
                        pv = _safe_float(r["_pnl"], 0)
                        pc = "#26a69a" if pv>=0 else "#ef5350"
                        bg = "#0D2B1E" if pv>=0 else "#2B0D0D"
                        sig= (r["_top_signal"][:30] if r["_top_signal"] else "—")
                        _cw.markdown(
                            f"<div style='display:flex;justify-content:space-between;"
                            f"align-items:center;padding:6px 10px;margin-bottom:2px;"
                            f"background:{bg};border-left:3px solid {pc};border-radius:3px'>"
                            f"<div><span style='font-size:.68rem;color:#434651;margin-right:5px'>#{ix}</span>"
                            f"<span style='font-family:Syne,sans-serif;font-weight:700;color:#d1d4dc'>{r['_sym']}</span>"
                            f"<span style='font-size:.67rem;color:#434651;margin-left:5px'>{r['_scan_date']}</span></div>"
                            f"<div style='text-align:right'>"
                            f"<span style='font-family:Syne,sans-serif;font-weight:800;color:{pc}'>{pv:+.2f}%</span>"
                            f"<div style='font-size:.63rem;color:#434651'>{sig}</div>"
                            f"</div></div>",
                            unsafe_allow_html=True,
                        )
            ...
            for r in _sa_rows:
                _an_export.append({
                    ...
                    "P&L %":      r["_pnl"],
                    "Status":     r["_status"],
                    "Hold Days":  r["_hold_days"],
                    "AI %":       r["_ai"],
                    "RSI":        r["_rsi"] or "—",
                    "ADX":        r["_adx"] or "—",
                    "Sector":     r["_sector"],
                    "Signal":     r["_top_signal"][:50] if r["_top_signal"] else "—",
                    "Source":     r["_method"],
                })
            st.download_button(
                "⬇️ Download Analytics CSV",
                data=pd.DataFrame(_an_export).to_csv(index=False).encode(),
                file_name=f"strategy_analytics_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                mime="text/csv", key="sa_csv",
            )
